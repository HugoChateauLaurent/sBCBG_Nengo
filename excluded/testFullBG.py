...
#------------------------------------------
# Unfinished modifications for Nengo. Switching to testPlausibility
#------------------------------------------
...
def createBG():
  ...
  create_pop = lambda *args, **kwargs: create(*args, **kwargs)
    ...
    if params['simulator'] == 'NEST':
      update_Ie = lambda p: nest.SetStatus(Pop[p],{"I_e":params['Ie'+p]})
  elif params['simulator'] == 'Nengo':
    def update_Ie(p):
      Ie_node = nengo.Node(params['Ie'+p])
      nengo.Connection(Ie_node, Pop[p].neurons, transform=[1/Pop[p].V_th], 
                        synapse=None)
  else:
    ...
    def update_Ie(p):
      # Is one Ie node for all channels fine?
      Ie_node = nengo.Node(params['Ie'+p])
      for i in range(len(Pop[p])):
        nengo.Connection(Ie_node, Pop[p][i].neurons, transform=[1/Pop[p][i].V_th], 
                          synapse=None)
...
def instantiate_BG(params={}, antagInjectionSite='none', antag=''):
  if not 'simulator' in params or params['simulator']=='Nengo':
    nest.ResetKernel()
    dataPath='log/'
    if 'nbcpu' in params:
