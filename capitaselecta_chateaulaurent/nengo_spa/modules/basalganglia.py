...
class BasalGanglia(Network):
    ...
    * Globus pallidus externus (*gpe*)
    ...
    def __init__(
            ...
            gaba_synapse=Default, sBCBG_params=None, **kwargs):
        ...
        if BasalGanglia.sBCBG:
            ...
            if sBCBG_params == None:
                sBCBG_params = {}
            sBCBG.nengo_instantiate(self.action_count, self, sBCBG_params if sBCBG_params != None else {})
            ...
            with self:
                ...
                for d in range(self.action_count):
                    nengo.Connection(scale[d], self.pops['CSN'][d],
                                    function=lambda x:10*x,
                                    synapse=.01, label='CSN input')

                # add bias input (BG performs best in the range 0.5--1.5)
                if abs(self.input_bias) > 0.0:
                    self.bias_input = nengo.Node(np.ones(self.action_count) * input_bias,
                ...
                for d in range(self.action_count):
                    ...
                    nengo.Connection(GPi_ens, self.output[d], synapse=nengo.synapses.Lowpass(filter_tau),
...
BasalGanglia.sBCBG = False # turn to True to use sBCBG instead of Gurney 