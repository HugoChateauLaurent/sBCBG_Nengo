...
#-------------------------------------------------------------------------------
def create(name, net, fake=False,parrot=True):
  if nbSim[name] == 0:
    ...
    if use_nest():
      ...
      with net:
        ...
        if not poisson:
          ...
          fake_ens = nengo.Ensemble(int(nbSim[name]), 1, 
          ...
          fake_ens = nengo.Ensemble(int(nbSim[name]), 1, 
                                        ...
                                        gain=np.ones((int(nbSim[name]))), 
                                        bias=np.zeros((int(nbSim[name]))),
                                        neuron_type=nengo.neurons.Poisson(),
                                        label=name)

        
        net.config[fake_ens].efferents = []
        net.config[fake_ens].Ie = nengo.Connection(nengo.Node([rate[name]], 
                                  label='Ie node '), fake_ens,
                                  synapse=None,
                                  label='Ie connection '+name)
        Pop[name] = fake_ens
        net.pops[name] = fake_ens
...
# parrot: do we use parrot neurons or not? If not, there will be no correlations in the inputs, and a waste of computation power...
#-------------------------------------------------------------------------------
def createMC(name,nbCh, net, fake=False,parrot=True):
  if nbSim[name] == 0:
    print('ERROR: create(): nbSim['+name+'] = 0')
    exit()
  ...
  if fake:
    ...
    fakeRate = [rate[name] for i in range(params['nbCh'])]
    ...
    if use_nest():
      ...
      with net:
        for i in range(nbCh):  
          ...
          net.config[fake_ens].efferents = []
          net.config[fake_ens].Ie = nengo.Connection(nengo.Node([fakeRate[i]], 
                                    label='Ie node '+name+' ch'+str(i)), fake_ens,
                                    synapse=None,
                                    label='Ie connection '+name+' ch'+str(i))
          Pop[name].append(fake_ens)
          net.pops[name].append(fake_ens)
...
def delayed_connection(pre, post, delay, transform, synapse, net):
  with net:
...
#------------------------------------------------------------------------------
def mass_connect(source, dest, synapse_label, inDegree, receptor_type, weight, delay, net, stochastic_delays=None, verbose=False):
  ...
  if integer_inDegree>0:      
    ...
    if use_nest():
      ...
      connection = delayed_connection(source, dest, delay, connectivity*nengo_weight, synapse, net)
...
#------------------------------------------------------------------------------
def mass_mirror(source, synapse_label, receptor_type, weight, delay, stochastic_delays, net, verbose=False):
  ...
  if ampa_conns:
    ...
    if use_nengo():
      for conn in ampa_conns:        
        ...
        connection = delayed_connection(source, conn.post.ensemble, delay, connectivity*nengo_weight, synapse, net)
...
#-------------------------------------------------------------------------------
def connect(type, nameSrc, nameTgt, redundancy, RedundancyType, net, LCGDelays=True, gain=1., stochastic_delays=None, verbose=False, projType=''):
  ...
  mass_connect(Pop[nameSrc], Pop[nameTgt], lbl, inDegree, recType[lRecType[0]], W[lRecType[0]], delay, stochastic_delays = stochastic_delays, net=net)
  if type == 'ex':
    # mirror the AMPA connection with similarly connected NMDA connections
    mass_mirror(Pop[nameSrc], lbl, recType['NMDA'], W['NMDA'], delay, stochastic_delays = stochastic_delays, net=net)
...
#                   Tgt channels:   (0) (1)
#-------------------------------------------------------------------------------
def connectMC(type, nameSrc, nameTgt, projType, redundancy, RedundancyType, net, LCGDelays=True, gain=1., source_channels = None, stochastic_delays=None, verbose=False):
  ...
  if projType == 'focused': # if projections focused, input come only from the same channel as tgtChannel
     for src_channel in source_channels: # for each relevant channel of the Source nucleus
       mass_connect(Pop[nameSrc][src_channel], Pop[nameTgt][src_channel-source_channels[0]], lbl, inDegree, recType[lRecType[0]], W[lRecType[0]], delay, stochastic_delays = stochastic_delays, net=net)
    ...
    for src_channel in source_channels: # for each relevant channel of the Source nucleus
      for tgt_channel in range(len(Pop[nameTgt])): # for each channel of the Target nucleus
        mass_connect(Pop[nameSrc][src_channel], Pop[nameTgt][tgt_channel], lbl, inDegree/len(Pop[nameTgt]), recType[lRecType[0]], W[lRecType[0]], delay, stochastic_delays = stochastic_delays, net=net)

  if type == 'ex':
    # mirror the AMPA connection with similarly connected NMDA connections
    for src_channel in source_channels: # for each relevant channel of the Source nucleus
      mass_mirror(Pop[nameSrc][src_channel], lbl, recType['NMDA'], W['NMDA'], delay, stochastic_delays = stochastic_delays, net=net)
...
def createBG(net):
  ...
  if params['nbCh'] == 1:
    ...
    if use_nest():
      update_Ie = lambda p, net: nest.SetStatus(Pop[p],{"I_e":params['Ie'+p]})
    elif use_nengo():
      def update_Ie(p, net):
    ...
    if use_nest():
      update_Ie = lambda p, net: [nest.SetStatus(Pop[p][i],{"I_e":params['Ie'+p]}) for i in range(len(Pop[p]))]
    elif use_nengo():
      def update_Ie(p, net):
  ...
  create_pop('MSN', net=net)
  update_Ie('MSN', net=net)

  nbSim['FSI'] = params['nbFSI']
  create_pop('FSI', net=net)
  update_Ie('FSI', net=net)

  nbSim['STN'] = params['nbSTN']
  create_pop('STN', net=net)
  update_Ie('STN', net=net)

  if params['splitGPe']:
    nbSim['Arky'] = params['nbArky']
    create_pop('Arky', net=net)
    update_Ie('Arky', net=net)
  
    nbSim['Prot'] = params['nbProt']
    create_pop('Prot', net=net)
    update_Ie('Prot', net=net)
  else:
    nbSim['GPe'] = params['nbGPe']
    create_pop('GPe', net=net)
    update_Ie('GPe', net=net)
      
      
  nbSim['GPi'] = params['nbGPi']
  create_pop('GPi', net=net)
  update_Ie('GPi', net=net)
  ...
  create_pop('CSN', nbCh=CSNchannels, fake=True, parrot=parrot, net=net)

  nbSim['PTN'] = params['nbPTN']
  create_pop('PTN', fake=True, parrot=parrot, net=net)

  nbSim['CMPf'] = params['nbCMPf']
  create_pop('CMPf', fake=True, parrot=params['parrotCMPf'], net=net) # was: False
...
#------------------------------------------
def connectBG(antagInjectionSite,antag, net):
  ...
  if 'nbCues' not in list(params.keys()):
    ...
    CSN_MSN = connect_pop('ex','CSN','MSN', net=net, projType=params['cTypeCSNMSN'], redundancy=params['redundancyCSNMSN'], gain=params['GCSNMSN'])
  else:
    # special case: extra 'cue' channels that target MSN
    CSN_MSN = connect_pop('ex','CSN','MSN', net=net, projType=params['cTypeCSNMSN'], redundancy=params['redundancyCSNMSN'], gain=Params['GCSNMSN']/2., source_channels=list(range(params['nbCh'])))
    connect_pop('ex','CSN','MSN', net=net, projType='diffuse', redundancy=params['redundancyCSNMSN'], gain=params['GCSNMSN']/2., source_channels=list(range(params['nbCh'], params['nbCh']+params['nbCues'])))
  PTN_MSN = connect_pop('ex','PTN','MSN', net=net, projType=params['cTypePTNMSN'], redundancy= params['redundancyPTNMSN'], gain=params['GPTNMSN'])
  CMPf_MSN = connect_pop('ex','CMPf','MSN', net=net,projType=params['cTypeCMPfMSN'],redundancy= params['redundancyCMPfMSN'],gain=params['GCMPfMSN'])
  connect_pop('in','MSN','MSN', net=net, projType=params['cTypeMSNMSN'], redundancy= params['redundancyMSNMSN'], gain=params['GMSNMSN'])
  connect_pop('in','FSI','MSN', net=net, projType=params['cTypeFSIMSN'], redundancy= params['redundancyFSIMSN'], gain=params['GFSIMSN'])
  # some parameterizations from LG14 have no STN->MSN or GPe->MSN synaptic contacts
  if alpha['STN->MSN'] != 0:
    print("alpha['STN->MSN']",alpha['STN->MSN'])
    connect_pop('ex','STN','MSN', net=net, projType=params['cTypeSTNMSN'], redundancy= params['redundancySTNMSN'], gain=params['GSTNMSN'])
  if alpha['GPe->MSN'] != 0:
    if params['splitGPe']:
      print("alpha['Arky->MSN']",alpha['Arky->MSN'])
      connect_pop('in','Arky','MSN', net=net, projType=params['cTypeArkyMSN'], redundancy= params['redundancyArkyMSN'], gain=params['GArkyMSN'])
    else:
      print("alpha['GPe->MSN']",alpha['GPe->MSN'])
      connect_pop('in','GPe','MSN', net=net, projType=params['cTypeGPeMSN'], redundancy= params['redundancyGPeMSN'], gain=params['GGPeMSN'])

  print('* FSI Inputs')
  connect_pop('ex','CSN','FSI', net=net, projType=params['cTypeCSNFSI'], redundancy= params['redundancyCSNFSI'], gain=params['GCSNFSI'])
  connect_pop('ex','PTN','FSI', net=net, projType=params['cTypePTNFSI'], redundancy= params['redundancyPTNFSI'], gain=params['GPTNFSI'])
  if alpha['STN->FSI'] != 0:
    connect_pop('ex','STN','FSI', net=net, projType=params['cTypeSTNFSI'],redundancy= params['redundancySTNFSI'],gain=params['GSTNFSI'])
  if params['splitGPe']:
    connect_pop('in','Arky','FSI', net=net, projType=params['cTypeArkyFSI'], redundancy= params['redundancyArkyFSI'], gain=params['GArkyFSI'])
  else:
    connect_pop('in','GPe','FSI', net=net, projType=params['cTypeGPeFSI'], redundancy= params['redundancyGPeFSI'], gain=params['GGPeFSI'])
    
  connect_pop('ex','CMPf','FSI', net=net,projType=params['cTypeCMPfFSI'],redundancy= params['redundancyCMPfFSI'],gain=params['GCMPfFSI'])
  connect_pop('in','FSI','FSI', net=net, projType=params['cTypeFSIFSI'], redundancy= params['redundancyFSIFSI'], gain=params['GFSIFSI'])

  print('* STN Inputs')
  connect_pop('ex','PTN','STN', net=net, projType=params['cTypePTNSTN'], redundancy= params['redundancyPTNSTN'],  gain=params['GPTNSTN'])
  connect_pop('ex','CMPf','STN', net=net,projType=params['cTypeCMPfSTN'],redundancy= params['redundancyCMPfSTN'], gain=params['GCMPfSTN'])
  if params['splitGPe']:
    connect_pop('in','Prot','STN', net=net, projType=params['cTypeProtSTN'], redundancy= params['redundancyProtSTN'],  gain=params['GProtSTN'])
  else:
    connect_pop('in','GPe','STN', net=net, projType=params['cTypeGPeSTN'], redundancy= params['redundancyGPeSTN'],  gain=params['GGPeSTN'])
  ...
  if params['splitGPe']:
      ...
      if antagInjectionSite == 'GPe':
        if   antag == 'AMPA':
          connect_pop('NMDA','CMPf','Arky', net=net,projType=params['cTypeCMPfArky'],redundancy= params['redundancyCMPfArky'],gain=params['GCMPfArky'])
          connect_pop('NMDA','STN','Arky', net=net, projType=params['cTypeSTNArky'], redundancy= params['redundancySTNArky'], gain=params['GSTNArky'])
          connect_pop('in','MSN','Arky', net=net,   projType=params['cTypeMSNArky'], redundancy= params['redundancyMSNArky'], gain=params['GMSNArky'])
          connect_pop('in','Prot','Arky', net=net, projType=params['cTypeProtArky'], redundancy= params['redundancyProtArky'], gain=params['GProtArky'])
          connect_pop('in', Arky_recurrent_source, 'Arky', net=net, projType=params['cTypeArkyArky'], redundancy= params['redundancyArkyArky'], gain=params['GArkyArky'])
        elif antag == 'NMDA':
          connect_pop('AMPA','CMPf','Arky', net=net,projType=params['cTypeCMPfArky'],redundancy= params['redundancyCMPfArky'],gain=params['GCMPfArky'])
          connect_pop('AMPA','STN','Arky', net=net, projType=params['cTypeSTNArky'], redundancy= params['redundancySTNArky'], gain=params['GSTNArky'])
          connect_pop('in','MSN','Arky', net=net,   projType=params['cTypeMSNArky'], redundancy= params['redundancyMSNArky'], gain=params['GMSNArky'])
          connect_pop('in','Prot','Arky', net=net, projType=params['cTypeProtArky'], redundancy= params['redundancyProtArky'], gain=params['GProtArky'])
          connect_pop('in', Arky_recurrent_source, 'Arky', net=net, projType=params['cTypeArkyArky'], redundancy= params['redundancyArkyArky'], gain=params['GArkyArky'])
          ...
          connect_pop('ex','CMPf','Arky', net=net,projType=params['cTypeCMPfArky'],redundancy= params['redundancyCMPfArky'],gain=params['GCMPfArky'])
          connect_pop('ex','STN','Arky', net=net, projType=params['cTypeSTNArky'], redundancy= params['redundancySTNArky'], gain=params['GSTNArky'])
        else:
      ...
      if antagInjectionSite == 'GPe':
        if   antag == 'AMPA':
          connect_pop('NMDA','CMPf','Prot', net=net, projType=params['cTypeCMPfProt'],redundancy= params['redundancyCMPfProt'],gain=params['GCMPfProt'])
          connect_pop('NMDA','STN','Prot', net=net,  projType=params['cTypeSTNProt'], redundancy= params['redundancySTNProt'], gain=params['GSTNProt'])
          connect_pop('in','MSN','Prot', net=net,    projType=params['cTypeMSNProt'], redundancy= params['redundancyMSNProt'], gain=params['GMSNProt'])
          connect_pop('in','Arky','Prot', net=net,    projType=params['cTypeArkyProt'], redundancy= params['redundancyArkyProt'], gain=params['GArkyProt'])
          ...
          connect_pop('AMPA','CMPf','Prot', net=net, projType=params['cTypeCMPfProt'],redundancy= params['redundancyCMPfProt'],gain=params['GCMPfProt'])
          connect_pop('AMPA','STN','Prot', net=net,  projType=params['cTypeSTNProt'], redundancy= params['redundancySTNProt'], gain=params['GSTNProt'])
          connect_pop('in','MSN','Prot', net=net,    projType=params['cTypeMSNProt'], redundancy= params['redundancyMSNProt'], gain=params['GMSNProt'])
          connect_pop('in','Arky','Prot', net=net,    projType=params['cTypeArkyProt'], redundancy= params['redundancyArkyProt'], gain=params['GArkyProt'])
          connect_pop('in', Prot_recurrent_source, 'Prot', net=net,  projType=params['cTypeProtProt'], redundancy= params['redundancyProtProt'], gain=params['GProtProt'])
        elif antag == 'AMPA+GABAA':
          connect_pop('NMDA','CMPf','Prot', net=net, projType=params['cTypeCMPfProt'],redundancy= params['redundancyCMPfProt'],gain=params['GCMPfProt'])
          connect_pop('NMDA','STN','Prot', net=net,  projType=params['cTypeSTNProt'], redundancy= params['redundancySTNProt'], gain=params['GSTNProt'])
        elif antag == 'GABAA':
          connect_pop('ex','CMPf','Prot', net=net, projType=params['cTypeCMPfProt'],redundancy= params['redundancyCMPfProt'],gain=params['GCMPfProt'])
          connect_pop('ex','STN','Prot', net=net,  projType=params['cTypeSTNProt'], redundancy= params['redundancySTNProt'], gain=params['GSTNProt'])
        else:
          print(antagInjectionSite,": unknown antagonist experiment:",antag)
      else:
        connect_pop('ex','CMPf','Prot', net=net, projType=params['cTypeCMPfProt'],redundancy= params['redundancyCMPfProt'],gain=params['GCMPfProt'])
      ...
      if antagInjectionSite == 'GPe':
        if   antag == 'AMPA':
          connect_pop('NMDA','CMPf','GPe', net=net, projType=params['cTypeCMPfGPe'],redundancy= params['redundancyCMPfGPe'],gain=params['GCMPfGPe'])
          connect_pop('NMDA','STN','GPe', net=net,  projType=params['cTypeSTNGPe'], redundancy= params['redundancySTNGPe'], gain=params['GSTNGPe'])
          connect_pop('in','MSN','GPe', net=net,    projType=params['cTypeMSNGPe'], redundancy= params['redundancyMSNGPe'], gain=params['GMSNGPe'])
          ...
          connect_pop('AMPA','CMPf','GPe', net=net, projType=params['cTypeCMPfGPe'],redundancy= params['redundancyCMPfGPe'],gain=params['GCMPfGPe'])
          connect_pop('AMPA','STN','GPe', net=net,  projType=params['cTypeSTNGPe'], redundancy= params['redundancySTNGPe'], gain=params['GSTNGPe'])
          connect_pop('in','MSN','GPe', net=net,    projType=params['cTypeMSNGPe'], redundancy= params['redundancyMSNGPe'], gain=params['GMSNGPe'])
          connect_pop('in', GPe_recurrent_source, 'GPe', projType=params['cTypeGPeGPe'], redundancy= params['redundancyGPeGPe'], gain=params['GGPeGPe'])
        elif antag == 'AMPA+GABAA':
          connect_pop('NMDA','CMPf','GPe', net=net, projType=params['cTypeCMPfGPe'],redundancy= params['redundancyCMPfGPe'],gain=params['GCMPfGPe'])
          ...
          connect_pop('ex','CMPf','GPe', net=net, projType=params['cTypeCMPfGPe'],redundancy= params['redundancyCMPfGPe'],gain=params['GCMPfGPe'])
          connect_pop('ex','STN','GPe', net=net,  projType=params['cTypeSTNGPe'], redundancy= params['redundancySTNGPe'], gain=params['GSTNGPe'])
        else:
          print(antagInjectionSite,": unknown antagonist experiment:",antag)
      else:
        connect_pop('ex','CMPf','GPe', net=net, projType=params['cTypeCMPfGPe'],redundancy= params['redundancyCMPfGPe'],gain=params['GCMPfGPe'])
        connect_pop('ex','STN','GPe', net=net,  projType=params['cTypeSTNGPe'], redundancy= params['redundancySTNGPe'], gain=params['GSTNGPe'])
        connect_pop('in','MSN','GPe', net=net,  projType=params['cTypeMSNGPe'], redundancy= params['redundancyMSNGPe'], gain=params['GMSNGPe'])
        connect_pop('in', GPe_recurrent_source, 'GPe', net=net, projType=params['cTypeGPeGPe'], redundancy= params['redundancyGPeGPe'], gain=params['GGPeGPe'])
  ...
  if antagInjectionSite =='GPi':
    if   antag == 'AMPA+NMDA+GABAA':
      ...
      connect_pop('in','MSN','GPi', net=net,    projType=params['cTypeMSNGPi'], redundancy= params['redundancyMSNGPi'], gain=params['GMSNGPi'])
      connect_pop('AMPA','STN','GPi', net=net,  projType=params['cTypeSTNGPi'], redundancy= params['redundancySTNGPi'], gain=params['GSTNGPi'])
      if params['splitGPe']:
        connect_pop('in','Prot','GPi', net=net,    projType=params['cTypeProtGPi'], redundancy= params['redundancyProtGPi'], gain=params['GProtGPi'])
      else:
        connect_pop('in','GPe','GPi', net=net,    projType=params['cTypeGPeGPi'], redundancy= params['redundancyGPeGPi'], gain=params['GGPeGPi'])
      connect_pop('AMPA','CMPf','GPi', net=net, projType=params['cTypeCMPfGPi'],redundancy= params['redundancyCMPfGPi'],gain=params['GCMPfGPi'])
      ...
      connect_pop('in','MSN','GPi', net=net,    projType=params['cTypeMSNGPi'], redundancy= params['redundancyMSNGPi'], gain=params['GMSNGPi'])
      connect_pop('NMDA','STN','GPi', net=net,  projType=params['cTypeSTNGPi'], redundancy= params['redundancySTNGPi'], gain=params['GSTNGPi'])
      if params['splitGPe']:
        connect_pop('in','Prot','GPi', net=net,    projType=params['cTypeProtGPi'], redundancy= params['redundancyProtGPi'], gain=params['GProtGPi'])
      else:
        connect_pop('in','GPe','GPi', net=net,    projType=params['cTypeGPeGPi'], redundancy= params['redundancyGPeGPi'], gain=params['GGPeGPi'])
      connect_pop('NMDA','CMPf','GPi', net=net, projType=params['cTypeCMPfGPi'],redundancy= params['redundancyCMPfGPi'],gain=params['GCMPfGPi'])
    elif antag == 'GABAA':
      connect_pop('ex','STN','GPi', net=net,  projType=params['cTypeSTNGPi'], redundancy= params['redundancySTNGPi'], gain=params['GSTNGPi'])
      connect_pop('ex','CMPf','GPi', net=net, projType=params['cTypeCMPfGPi'],redundancy= params['redundancyCMPfGPi'],gain=params['GCMPfGPi'])
    else:
      print(antagInjectionSite,": unknown antagonist experiment:",antag)
  else:
    connect_pop('in','MSN','GPi', net=net,  projType=params['cTypeMSNGPi'], redundancy= params['redundancyMSNGPi'], gain=params['GMSNGPi'])
...
#------------------------------------------
def instantiate_BG(net, params={}, antagInjectionSite='none', antag=''):
  ...
  createBG(net)
  return connectBG(antagInjectionSite,antag, net)
...
#------------------------------------------
def checkAvgFR(net, showRasters=False,params={},antagInjectionSite='none',antag='',logFileName='', computeLFP=False, computePS=False):
  if use_nest():
    nest.ResetNetwork()
        ...
        if params['nbCh']>1:
          ...
          for ch in range(params['nbCh']):
            ...
            if N not in ['CSN', 'PTN', 'CMPf']:
              probes[N][-1]['multimeters'] = nengo.Probe(Pop[N][ch].neurons, 'voltage')
              probes[N][-1]['inputs'] = nengo.Probe(Pop[N][ch].neurons, 'input')
              #probes[N][-1]['spikes'] = nengo.Probe(Pop[N][ch].neurons, 'spikes')

            probes[N][-1]['outputs'] = nengo.Probe(Pop[N][ch].neurons, 'output')
...
def main(): # used
  ...
  net = reset_net()
  instantiate_BG(net, params, antagInjectionSite='none', antag='')
  score = np.zeros((2))
  #mapTopology2D(show=True)
  score += checkAvgFR(net, params=params,antagInjectionSite='none',antag='',showRasters=False)
  ...
  if score[0] < score[1]:
    ...
    if params['nbCh'] == 1:
      ...
      for a in ['AMPA','AMPA+GABAA','NMDA','GABAA']:
        ww = deactivate('GPe', a)
        score += checkAvgFR(net, params=params,antagInjectionSite='GPe',antag=a)
        reactivate('GPe', a, ww)

      for a in ['AMPA+NMDA+GABAA','AMPA','NMDA+AMPA','NMDA','GABAA']:
        ww = deactivate('GPi', a)
        score += checkAvgFR(net, params=params,antagInjectionSite='GPi',antag=a)
        reactivate('GPi', a, ww)
    else:
      ...
      for a in ['AMPA','AMPA+GABAA','NMDA','GABAA']:
        net = reset_net()
        instantiate_BG(net, params, antagInjectionSite='GPe', antag=a)
        score += checkAvgFR(net, params=params,antagInjectionSite='GPe',antag=a)

      for a in ['AMPA+NMDA+GABAA','AMPA','NMDA+AMPA','NMDA','GABAA']:
        net = reset_net()
        instantiate_BG(net, params, antagInjectionSite='GPi', antag=a)
        score += checkAvgFR(net, params=params,antagInjectionSite='GPi',antag=a)
  else:
...
#-----------------------------------------------------------------------
def nengo_instantiate(dimensions, net, new_params={}):
