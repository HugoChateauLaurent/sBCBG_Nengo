    ...
    if use_nest():
      ...
      for i in range(nbCh):
        Pop[name].append(nengo.Ensemble(int(nbSim[name]), 1, encoders=np.ones((int(nbSim[name]))), 
        													...
        													label=name+' ch'+str(i)))
        # List of afferent connections with Ie=0 node:
		    Pop[name][-1].afferents = [nengo.Connection(nengo.Node([0], label='Ie node '+name+' ch'+str(i)), Pop[name][-1],transform=1./Pop[name][-1].neuron_type.params['V_th'], synapse=None, label='Ie connection '+name+' ch'+str(i))]
...
# Assumption: a cell from pre cannot be connected twice to a cell from post
#------------------------------------------------------------------------------
def connectivity_matrix(integer_inDegree, n_pre, n_post):
  connectivity = np.zeros((n_post, n_pre))
  for post_neuron in range(n_post):
    for in_i in range(integer_inDegree):
      count = 0
      while count==0 or connectivity[post_neuron, pre_neuron] != 0:
        pre_neuron = rnd.randint(0, n_pre)
        count+=1
        if count>1000:
          print('Nengo warning: can\'t find a free pre neuron')
      connectivity[post_neuron, pre_neuron] += 1

  if (connectivity>1).any() or (connectivity<0).any():
    print('Nengo error: connectivity matrix with element other than 0 or 1')
  return connectivity
...
def mass_connect(source, dest, synapse_label, inDegree, receptor_type, weight, delay, stochastic_delays=None, verbose=False):
  ...
  if integer_inDegree>0:
    ...
    if use_nest():
      ...
      dest.afferents.append(nengo.Connection(source.neurons, dest.neurons, 
      						transform=connectivity_matrix()*weight*exp(1)*dest.neuron_type.params['tau_syn'][receptor_type]/1000./dest.neuron_type.params['V_th'], 
      						synapse=Alpha(dest.neuron_type.params['tau_syn'][receptor_type], label=synapse_label)))
  ...
  if remaining_connections > 0:
    if use_nest():
      ...
      if stochastic_delays:
        raise NotImplementedError("TODONengo: delay distribution")
      dest.afferents.append(nengo.Connection(source.neurons, dest.neurons, 
            					transform=[weight*exp(1)*dest.neuron_type.params['tau_syn'][receptor_type]/1000./dest.neuron_type.params['V_th']], 
        						Alpha(dest.neuron_type.params['tau_syn'][receptor_type], label=synapse_label)))
...
initNeurons() # sets the default params of iaf_psc_alpha_mutisynapse neurons to CommonParams
...
#---------------------------
# Nengo implementation not needed because not used 
#---------------------------
def main(): 
